...
export LOGNAME="resnet50-topK${TOPK}_decay${TOPKD}-Knn${N}-ALPHA${ALPHA}-BETA${BETA}-GAMMA${GAMMA}-DELTA${DELTA}-TAUM${TAU_M}-NoConfWeight"

CUDA_VISIBLE_DEVICES=${GPU} python main.py --seed ${SEED} \
        --cfg ${CONFIG} --gpu 0 --log-proj ${LOGPROJ} --log-name ${LOGNAME} \
        --arch resnet50 --opt sgd --batch-size 64 --lr 0.01 --warmup-lr 0.01 --lr-decay cosine:10 --weight-decay 3e-4 --eps 0.1 --epochs 120 \
